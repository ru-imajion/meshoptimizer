...
void decomposeTransform(cgltf_float translation[3], cgltf_float rotation[4], cgltf_float scale[3], const cgltf_float* transform)
{
	...
	cgltf_float sign = (det < 0.f) ? -1.f : 1.f;

	// recover scale from axis lengths
	scale[0] = gp_sqrt(m[0][0] * m[0][0] + m[0][1] * m[0][1] + m[0][2] * m[0][2]) * sign;
	scale[1] = gp_sqrt(m[1][0] * m[1][0] + m[1][1] * m[1][1] + m[1][2] * m[1][2]) * sign;
	scale[2] = gp_sqrt(m[2][0] * m[2][0] + m[2][1] * m[2][1] + m[2][2] * m[2][2]) * sign;

	// normalize axes to get a pure rotation matrix
	cgltf_float rsx = (scale[0] == 0.f) ? 0.f : 1.f / scale[0];
	cgltf_float rsy = (scale[1] == 0.f) ? 0.f : 1.f / scale[1];
	cgltf_float rsz = (scale[2] == 0.f) ? 0.f : 1.f / scale[2];

	cgltf_float r00 = m[0][0] * rsx, r10 = m[1][0] * rsy, r20 = m[2][0] * rsz;
	cgltf_float r01 = m[0][1] * rsx, r11 = m[1][1] * rsy, r21 = m[2][1] * rsz;
	cgltf_float r02 = m[0][2] * rsx, r12 = m[1][2] * rsy, r22 = m[2][2] * rsz;

	// "branchless" version of Mike Day's matrix to quaternion conversion
	int qc = r22 < 0 ? (r00 > r11 ? 0 : 1) : (r00 < -r11 ? 2 : 3);
	cgltf_float qs1 = qc & 2 ? -1.f : 1.f;
	cgltf_float qs2 = qc & 1 ? -1.f : 1.f;
	cgltf_float qs3 = (qc - 1) & 2 ? -1.f : 1.f;

	cgltf_float qt = 1.f - qs3 * r00 - qs2 * r11 - qs1 * r22;
	cgltf_float qs = 0.5f / gp_sqrt(qt);
